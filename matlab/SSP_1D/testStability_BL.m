...
% Burgers Flux
flux = @(w) w.^2/(w.^2 + (1 - w).^2); 
dflux = @(w) -2*(w - 1).*w./(2*w.^2- 2*w + 1).^2;
fprintf('Buckey Leverett Flux:\n');   
...
U_ssp2 = u0; U_rk2 = u0;
dt = CFLS(j)*dx/max(abs(dflux(U_ssp2)));
...
for j = 1:length(CFLS)
U_ssp3 = u0; U_rk3 = u0; 
dt = CFLS(j)*dx/max(abs(dflux(U_ssp3)));
figure('Units','characters','Position',[0 0 120 120])  
...
U_ssp4 = u0; U_rk4 = u0;
dt = CFLS(j)*dx/max(abs(dflux(U_ssp4)));
figure('Units','characters','Position',[0 0 120 120])  
