/* Explicit 2nd-order SSP time stepper class implementation file.
...
// The explicit 2nd-order SSP time step evolution routine
//
...
//               [t0, t1, ..., tN]
std::vector<double> SSP2Stepper::Evolve(std::vector<double>& tspan, double h, 
                                        std::vector<double>& y) {
...
// Single step of explicit 2nd-order SSP
//
// Inputs:  t holds the current time
//          h holds the current time step size
//          z, f1-f2 hold temporary vectors needed for the problem
...
int SSP2Stepper::Step(double t, double h, std::vector<double>& y) {
