...
int main() {
  ...
  for(int ii = 0; ii < CFLS2.size(); ii++){
      ...
      if(ii == 0){
        Matrix t = Linspace(t0, Tf, 1, Nt+1);
        Matrix xt(1, x.size());
        Matrix yo(1, y0.size());
        ...
        t.Write("t_Nonlinear.txt");
        yo.Write("y0_Nonlinear.txt");
      }
      
      char fname[35];
      sprintf(fname,"ycalc_NonlinearBR_erk2_CFL(%g).txt",CFL);
      ycal.Write(fname);
    }
    ...
    for(int ii = 0; ii < CFLS2.size(); ii++){
      ...
      char fname[35];
      sprintf(fname,"ycalc_NonlinearBR_ssp2_CFL(%g).txt",CFL);
      ycal.Write(fname);
    }
    ...
    for(int ii = 0; ii < CFLS3.size(); ii++){
      ...
      char fname[35];
      sprintf(fname,"ycalc_NonlinearBR_erk3_CFL(%g).txt",CFL);
      ycal.Write(fname);
    }
    ...
    char fname[35];
    sprintf(fname,"ycalc_NonlinearBR_ssp3_CFL(%g).txt",CFL);
    ycal.Write(fname);
  ...
  for(int ii = 0; ii < CFLS4.size(); ii++){
      ...
      for (int istep=0; istep<Nt; istep++) {
       ...
       char fname[35];
       sprintf(fname,"ycalc_NonlinearBR_erk4_CFL(%g).txt",CFL);
       ycal.Write(fname);
    }
    ...
    char fname[35];
    sprintf(fname,"ycalc_NonlinearBR_ssp4_CFL(%.3f).txt",CFL);
    ycal.Write(fname);
